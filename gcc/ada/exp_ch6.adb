...
package body Exp_Ch6 is
   ...
   procedure Set_Enclosing_Sec_Stack_Return
     (N : Node_Id; Skip_Outlined : Boolean := True);
   ...
   procedure Expand_Simple_Function_Return (N : Node_Id) is
      ...
      Return_Needs_Finalization : constant Boolean := Present (Utyp)
        and then Needs_Finalization (Utyp)
        and then not (Exp_Is_Function_Call
                       and then Needs_Finalization (Exp_Typ));

      Return_Requires_SS : constant Boolean :=
        Needs_Secondary_Stack (R_Type)
          or else Is_Secondary_Stack_Thunk (Scope_Id);
   ...
   begin
      ...
      --  secondary stack (or any other one).
        ...
        and then not (Return_Needs_Finalization
      ...
      then
         ...
         elsif Return_Needs_Finalization then
            declare
   ...
   begin
      ...
      while Present (P) loop
         ...
         --  Mark the enclosing function
            ...
            --  Do not go beyond the enclosing function unless we are in
            --  a procedure generated by parallel construct.

            exit when not Skip_Outlined
              or else Ekind (Defining_Entity (P)) /= E_Procedure
              or else not Is_Outlined_Parallel (Defining_Entity (P));
         end if;
