...
Begin.
    ...
    PERFORM WITH TEST AFTER UNTIL EndOfInput
       ...
       END-PERFORM
    END-PERFORM
    DISPLAY "Looking for  ZZ with NumberOfCountries set to " NumberOfCountries
    MOVE "ZZ" TO CountryCodeIn
    PERFORM SearchCountryCodeTable

    ADD 1 TO NumberOfCountries
...
LoadCountryCodeTable.
    ...
    PERFORM VARYING NumberOfCountries FROM 1 BY 1 UNTIL EndOfCountryCodeFile
       ...
       END-READ
    END-PERFORM
    DISPLAY "We read in " NumberOfCountries " countries"
    ADD 1 TO NumberOfCountries GIVING plus1
    DISPLAY "We are writing ZZ to Country("   plus1   ")" " While NumberOfCountries is " NumberOfCountries
    MOVE "ZZ **** FOUND ****" TO Country(plus1)
